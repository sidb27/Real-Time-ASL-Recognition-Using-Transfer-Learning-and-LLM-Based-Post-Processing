...
detects the hand region from webcam frames,
classifies the detected sign,
accumulates character predictions,
refines the output into more meaningful text using an LLM.
Methodology
...
Because the public datasets had different original resolutions, all images were resized to a common size before training. Standard normalization was applied using ImageNet statistics, and augmentation techniques were used to improve generalization.
...
This helped evaluate how well the trained models handled visual perturbations.
...
Crop/localize the hand region
Preprocess the crop for the classifier
Run sign classification using the fine-tuned ResNet18 model
Accumulate predictions over time
Use Llama-based post-processing to convert the sequence into more meaningful output
...
One of the biggest findings of this project was that high offline accuracy did not automatically translate to strong webcam performance.
